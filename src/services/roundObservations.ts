...
export class RoundObservationsService {
  ...
  async getLatestObservation(
    roundId: number,
    type: 'wallet-balances' | 'price-update' | 'game-event' = 'wallet-balances'
  ): Promise<RoomOperationResult<Tables<'round_observations'> | null>> {
    try {
      const { data: observation, error } = await supabase
        .from('round_observations')
        .select('*')
  ...
  //TODO Recent type changes broke this. If you see this and need it, please fix or ping @ad0ll
  ...
  //     return Object.keys(currentBalances).reduce(
  //       (acc, agentId) => {
  ...
  //     const nativeChange =
  //       ((parseFloat(currentBalance.nativeValue) - parseFloat(previousBalance.nativeValue)) /
  //         parseFloat(previousBalance.nativeValue)) *
  //       100;

  //     const usdChange =
  //       ((parseFloat(currentBalance.usdValue) - parseFloat(previousBalance.usdValue)) /
  //         parseFloat(previousBalance.usdValue)) *
  //       100;
  ...
  //TODO Function shouldn't be needed, signature auth happens in entrypoints
